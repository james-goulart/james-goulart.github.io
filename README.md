...
- Built QuintoAndar's first LLM product in search, combining natural language, visual retrieval, and qualification logic
- Improved signed contracts through search evolutions, with especially strong leverage in the for-sale marketplace
- Helped drive 9x growth in for-sale signed contracts through operating-model and funnel redesign
- Built financial add-ons that improved rental profitability and reached meaningful adoption
- Redesigned lending and investment flows in fintech with large conversion and operational gains
...
- `index.html` and page entrypoints: static site shell
- `data.js`: portfolio content model, experiences, and cases
- `portfolio.js`: rendering logic for home, cases, experiences, and navigation
- `chat.js`: portfolio copilot experience
- `styles.css`: presentation layer
- `docs/`: positioning and copy drafts for future iterations

## Why the site has a copilot

Most portfolios are optimized for passive reading.

This one is also optimized for active exploration: a recruiter, hiring manager, or peer can ask direct questions about AI experience, leadership scope, case studies, or measurable outcomes.

The copilot is not the main proof. The underlying cases are.
...
The deployed site is available through GitHub Pages for this repository.

## Local development

This is a lightweight static portfolio setup.

To iterate locally, serve the repository with a local web server instead of opening files directly in the browser.
...
- demote the copilot from hero to supporting element
